...
//		double scale = 0.8;
	...
	public List<Line2d> getLines(){
        ...
        double [] out = lsd(imageData, width, height);
...
//		imageData = new double[width * height];
		...
		imageData = bf.getData().getPixels(0,0,width,height,new double[width*height*3]);
...
//		System.out.println(imageData.length);
		...
		for(int i = 0; i < imageData.length-3; i+=3) {
			double B = imageData[i];
			double G = imageData[i+1];
			double R = imageData[i+2];
	...
	public LSD(File fin, float scaleFactor) throws IOException {

		BufferedImage myPicture = ImageIO.read(fin);
		int x = myPicture.getWidth();
		int y = myPicture.getHeight();

		imageData = myPicture.getData().getPixels(0,0,x,y,new double[x*y*3]);
		width = x;
		height = y;
		scale = scaleFactor;
